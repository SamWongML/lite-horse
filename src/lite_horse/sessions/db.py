...
class SessionDB:
    ...
    def copy_messages(self, *, src_session_id: str, dst_session_id: str) -> int:
        """Copy every message row from ``src`` to ``dst``. Returns count copied.

        Timestamps and roles are preserved; ids are regenerated by the new
        ``AUTOINCREMENT`` inserts. Caller is responsible for creating the
        destination session first. Used by ``/fork``.
        """
        with self._writer() as c:
            rows = c.execute(
    ...
    def list_recent_sessions(
        ...
        """Return session rows ordered by ``started_at`` descending.

        Shared by ``/resume`` (REPL picker) and ``litehorse sessions list``.
        """
        sql = (
            "SELECT id, source, user_id, model, started_at, ended_at, end_reason, "
            "message_count, title FROM sessions"
        )
