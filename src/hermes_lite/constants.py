"""Global constants and path helpers for hermes-lite."""
...
def hermeslite_home() -> Path:
    """Return the state directory, honoring HERMESLITE_HOME override."""
    return Path(os.environ.get("HERMESLITE_HOME", Path.home() / ".hermeslite"))


# Hermes-compatible char limits — DO NOT change without updating prompt expectations.
...
# SQLite schema version for our own session DB (not Hermes's numbering).
SCHEMA_VERSION = 1
